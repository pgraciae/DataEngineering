...
def GoodPath( Cities ):
    ...
    del NotVisited[0]
    N = len(NotVisited)
    Cx = np.array([c.x for c in NotVisited], dtype=np.float32) #treiem fora del while les funcions de creació d'arrays per evitar crear-ne tantes com ciutats
    Cy = np.array([c.y for c in NotVisited], dtype=np.float32) #amb el qual guanyem tant memòria 
    while N > 0:
        City = Path[-1]
        x, y = City.x, City.y
        pos = closerCity( x, y, N, Cx[0:N], Cy[0:N] )
        Path.append( NotVisited[pos] )        
        NotVisited[pos] = NotVisited[N-1]
        Cx[pos] = Cx[N-1]
        Cy[pos] = Cy[N-1]        
        N -= 1
